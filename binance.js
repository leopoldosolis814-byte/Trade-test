...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'chat') {
      ...
      const { message, symbols, lang } = req.body;
      const instruccionIdioma = lang === 'en'
        ? 'Respond ONLY in English, regardless of what language the user writes in.'
        : 'Respondé SIEMPRE en español, sin importar en qué idioma escriba el usuario.';
        ...
        try {
          const groqRes = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
              model: 'openai/gpt-oss-20b',
              messages: [
                { role: 'system', content: `Eres un asistente de trading profesional. Respondés breve y estructurado. ${instruccionIdioma} Comparás los activos pedidos usando SOLO los datos reales que te paso, sin inventar. Nunca das certezas del futuro, solo lecturas de probabilidad. No ejecutás órdenes.${contextoMulti}` },
      ...
      try {
        const groqRes = await fetch('https://api.groq.com/openai/v1/chat/completions', {
          ...
          body: JSON.stringify({
            model: 'openai/gpt-oss-20b',
            messages: [
              { role: 'system', content: `Eres un asistente de trading profesional. ${instruccionIdioma} Respondé de forma clara, breve y estructurada. Nunca des certezas sobre el futuro del precio, solo lecturas de probabilidad basadas en los datos que tenés. No ejecutás órdenes, solo sugerís y explicás.
