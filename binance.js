...
function backtestEMACross(closes, highs, lows, opts = {}) {
  ...
  const entry = i => ema9[i - 1] != null && ema9[i - 1] <= ema21[i - 1] && ema9[i] > ema21[i];
  const exit = i => ema9[i - 1] != null && ema9[i - 1] >= ema21[i - 1] && ema9[i] < ema21[i];
...
function backtestRSIExtremos(closes, highs, lows, opts = {}) {
  ...
  const entry = i => rsi[i] != null && rsi[i] < 30;
  const exit = i => rsi[i] != null && rsi[i] > 70;
  return runBacktestEngine(closes, highs, lows, entry, exit, opts);
}
