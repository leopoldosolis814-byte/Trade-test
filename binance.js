...
function resumenBacktest(trades, balance, balanceInicial) {
...
function backtestEMACross(closes, balanceInicial = 10000) {
  ...
  let posicion = null, balance = balanceInicial;
  const trades = [];
  ...
  return resumenBacktest(trades, balance, balanceInicial);
}

// ESTRATEGIA 2: RSI en extremos (compra sobreventa <30, vende sobrecompra >70)
function backtestRSIExtremos(closes, balanceInicial = 10000) {
  ...
  for (let i = 15; i < closes.length; i++) {
    ...
    if (!posicion && rsi[i] < 30) {
      posicion = { entryPrice: closes[i] };
    } else if (posicion && rsi[i] > 70) {
      const pnlPct = (closes[i] - posicion.entryPrice) / posicion.entryPrice;
      balance *= (1 + pnlPct);
      trades.push({ entrada: Number(posicion.entryPrice.toFixed(2)), salida: Number(closes[i].toFixed(2)), pnlPct: Number((pnlPct * 100).toFixed(2)) });
      posicion = null;
...
// ESTRATEGIA 3: Bandas de Bollinger (compra cerca de banda inferior, vende cerca de superior)
function backtestBollinger(closes, balanceInicial = 10000) {
  ...
  for (let i = 20; i < closes.length; i++) {
    ...
    if (!posicion && closes[i] < lower[i]) {
      posicion = { entryPrice: closes[i] };
    } else if (posicion && closes[i] > upper[i]) {
      const pnlPct = (closes[i] - posicion.entryPrice) / posicion.entryPrice;
      balance *= (1 + pnlPct);
      trades.push({ entrada: Number(posicion.entryPrice.toFixed(2)), salida: Number(closes[i].toFixed(2)), pnlPct: Number((pnlPct * 100).toFixed(2)) });
      posicion = null;
    ...
    if (action === 'backtest') {
      ...
      if (strategy === 'todas') {
        ...
        for (const [key, { nombre, fn }] of Object.entries(estrategias)) {
          resultados[key] = { estrategia: nombre, ...fn(closes) };
        }
        return res.json({ success: true, symbol, interval, velasAnalizadas: closes.length, comparacion: resultados });
      }
      ...
      const resultado = elegida.fn(closes);
      return res.json({ success: true, symbol, interval, velasAnalizadas: closes.length, estrategia: elegida.nombre, ...resultado });
    }
